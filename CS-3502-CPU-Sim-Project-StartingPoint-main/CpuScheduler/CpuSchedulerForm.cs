...
4. Compare algorithm behaviors and outcomes";
        ...
        private List<SchedulingResult> RunSRTFAlgorithm(List<ProcessData>processes)
        {
            ...
            var remainingBurstTimes = new Dictionary<string, int>();
            var processResults = new Dictionary<string, SchedulingResult>();
            // Initialize remaining burst time and results
            foreach (var process in processes)
            {
                remainingBurstTimes[process.ProcessID] = process.BurstTime;
                processResults[process.ProcessID] = new SchedulingResult
                ...
                // handles the edge case where burst = 0 and breaks SRTF
                if (process.BurstTime == 0)
                {
                    var r = processResults[process.ProcessID];
                    r.StartTime = process.ArrivalTime;
                    r.FinishTime = process.ArrivalTime;
                    r.TurnaroundTime = 0;
                    r.WaitingTime = 0;
                    completed++;
                }
            ...
            {
                var availableProcesses = processes.Where(p => p.ArrivalTime <= currentTime && remainingBurstTimes[p.ProcessID] > 0).ToList();
                // No process has arrived yet, jump to next arrival time
                if (availableProcesses.Count == 0)
                {
                    currentTime = Math.Max(currentTime, processes.Where(p => remainingBurstTimes[p.ProcessID] > 0).Min(p => p.ArrivalTime));
                    continue;
                }
                // Select next process with shortest remaining burst time, tiebreaker: arrival time
                // If arrival time is also the same, tiebreaker: processID
                var nextProcess = availableProcesses.OrderBy(p => remainingBurstTimes[p.ProcessID]).ThenBy(p => p.ArrivalTime).ThenBy(p => p.ProcessID).First();
                var result = processResults[nextProcess.ProcessID];
                ...
                currentTime++;
                // Once process completes, updates result with times and increments completed
        ...
        private List<SchedulingResult> RunHRRNAlgorithm(List<ProcessData>processes)
        {
            ...
            var remainingProcesses = processes.ToList();
            var results = new List<SchedulingResult>();
            // handles edge case where burst = 0
            foreach (var process in processes.Where(p => p.BurstTime == 0))
            {
                results.Add(new SchedulingResult
                {
                    ProcessID = process.ProcessID,
            ...
            while(remainingProcesses.Count() > 0) {
                ...
                foreach (var process in availableProcesses)
                {
                    ...
                    (RR == highestRR && process.ArrivalTime == nextProcess.ArrivalTime && 
                    string.Compare(process.ProcessID, nextProcess.ProcessID) < 0))
                    {
                        highestRR = RR;
                        nextProcess = process;
                    }
        ...
        private void DisplaySchedulingResults(List<SchedulingResult> results, string algorithmName)
        {
            ...
            var totalTime = results.Max(r => r.FinishTime);
            var cpuUtilization = totalBurst/(double)totalTime * 100;
            ...
            listView1.AutoResizeColumns(ColumnHeaderAutoResizeStyle.ColumnContent); // resize the columns to fit data
        ...
        /// <summary>
        /// Exports the result table.
        /// </summary>
        private void ExportResults_Click(object sender, EventArgs e)
        {
            if(listView1.Items.Count == 0) // no results
            {
                MessageBox.Show("No scheduling results to save. Please run an algorithm first.",
            ...
            using (var saveDialog = new SaveFileDialog())
            {
                ...
                saveDialog.Title = "Save CPU Scheduling Results";
                if (saveDialog.ShowDialog() == DialogResult.OK) 
                {
                    try 
                    {
                        using (var writer = new System.IO.StreamWriter(saveDialog.FileName))
                        {
                            ...
                            var headers = listView1.Columns.Cast<ColumnHeader>().Select(ch => ch.Text);
                            writer.WriteLine(string.Join(",", headers));
                            // Write data rows
                            foreach (ListViewItem item in listView1.Items) 
                            {
                                var row = item.SubItems.Cast<ListViewItem.ListViewSubItem>().Select(sub => sub.Text);
                                writer.WriteLine(string.Join(",", row));
                            }
                        }
                        MessageBox.Show($"Scheduling results saved successfully to:\n{saveDialog.FileName}",
        ...
        private void CpuSchedulerForm_Load(object sender, EventArgs e)
        {
            ...
            ApplyRoundedCorners(btnExportData);
        ...
        private void ApplyDarkTheme()
        {
            ...
            ApplyDarkThemeToSchedulerButton(btnExportData);
        ...
        private void ApplyLightTheme()
        {
            ...
            ApplyLightThemeToSchedulerButton(btnExportData);
            ...
            btnSRTF.BackColor = Color.MediumPurple;
            btnHRRN.BackColor = Color.IndianRed;
